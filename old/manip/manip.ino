...
#define power_OFF_while_STANDING false
...
void setup() {
  ...
  stepperY2.setMaxSpeed(MAX_SPEED);     // скорость движения к цели
  stepperY2.setAcceleration(ACCEL); // ускорение
  homing();
...
void loop() {
  ...
  if (stepperX.getStatus() != 0 && stepperY.getStatus() != 0 && stepperY2.getStatus() != 0) {
